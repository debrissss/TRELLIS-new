...
class SLatVaeMeshDecoderTrainer(BasicTrainer):
    ...
    def export_loss_for_file(self, loss):
        ...
        contribution = {}
        if 'reg_loss' in export:
            contribution['reg_loss'] = export['reg_loss']
        if 'tsdf_loss' in export:
            contribution['tsdf_loss'] = self.lambda_tsdf * export['tsdf_loss']
        if 'mask_loss' in export:
            contribution['mask_loss'] = export['mask_loss']
        if 'depth_loss' in export:
            contribution['depth_loss'] = self.lambda_depth * export['depth_loss']
        if 'normal_loss_perceptual' in export:
            contribution['normal_loss_perceptual'] = export['normal_loss_perceptual']
        if 'normal_map_loss_perceptual' in export:
            contribution['normal_map_loss_perceptual'] = self.lambda_color * export['normal_map_loss_perceptual']
        if 'color_loss_perceptual' in export:
            contribution['color_loss_perceptual'] = self.lambda_color * export['color_loss_perceptual']
