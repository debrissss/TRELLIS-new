...
class SLatVaeGaussianTrainer(BasicTrainer):
    ...
    def export_loss_for_file(self, loss):
        ...
        contribution = {}
        if 'l1' in export:
            contribution['l1'] = export['l1']
        if 'l2' in export:
            contribution['l2'] = export['l2']
        if 'ssim' in export:
            contribution['ssim'] = self.lambda_ssim * export['ssim']
        if 'lpips' in export:
            contribution['lpips'] = self.lambda_lpips * export['lpips']
        if 'kl' in export:
            contribution['kl'] = self.lambda_kl * export['kl']
        if 'reg_vol' in export and 'lambda_vol' in self.regularizations:
            contribution['reg_vol'] = self.regularizations['lambda_vol'] * export['reg_vol']
        if 'reg_opacity' in export and 'lambda_opacity' in self.regularizations:
            contribution['reg_opacity'] = self.regularizations['lambda_opacity'] * export['reg_opacity']
