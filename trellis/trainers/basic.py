...
class BasicTrainer(Trainer):
    ...
    # BEGIN detailed loss export: helpers for richer loss-only records in loss.txt.
    def export_loss_for_file(self, loss):
        """
        Convert the loss dict returned by training_losses into a detached,
        JSON-serializable snapshot for loss.txt. This must not mutate the
        original loss dict used by backpropagation or normal logging.
        """
    ...
    def _state_dicts_to_master_params(self, master_params, state_dicts):
        """
        将 state_dict 转换为主参数（Master parameters）。
        """
        master_params_names = sum(
            [[(name, n) for n, p in model.named_parameters() if p.requires_grad] for name, model in self.models.items()]
        , [])
    ...
    def load(self, load_dir, step=0):
        ...
        for name, model in self.models.items():
            ...
            # 如果使用 inflat_all FP16 模式，需要将加载的模型参数转换为半精度（FP16）
            if self.fp16_mode == 'inflat_all':
                model.convert_to_fp16()
        # 将加载好的状态参数同步更新到优化器对应的主参数（Master Parameters）中
        self._state_dicts_to_master_params(self.master_params, model_ckpts)
        del model_ckpts

        # 仅在主进程（Rank 0）上，循环加载各个 EMA 率对应的 EMA 模型参数
        if self.is_master:
        ...
        # 加载混杂状态文件（misc），恢复优化器、训练步数、数据采样器等控制状态，保障 Resume 状态一致性
        ...
        # BEGIN resume LR override: allow continuing training with a newly adjusted learning rate.
        # This keeps the resumed optimizer state such as Adam moments, while replacing only
        # param_group["lr"] when the config explicitly asks for it.
        if self.resume_optimizer_lr is not None:
            if self.resume_optimizer_lr == 'config':
                resume_lr = self.optimizer_config['args']['lr']
            else:
                resume_lr = float(self.resume_optimizer_lr)
            for param_group in self.optimizer.param_groups:
                param_group['lr'] = resume_lr
            if self.is_master:
                print(f' Override optimizer lr to {resume_lr}.', end='')
        # END resume LR override.
        ...
        # 在多 GPU 分布式环境下，阻断并同步所有卡，确保主备进程全部加载完成
        if self.world_size > 1:
            dist.barrier()
        if self.is_master:
            print(' Done.')

        # 同步完成后，执行 DDP 参数同步一致性检查，防范多卡权重分裂
        if self.world_size > 1:
            self.check_ddp()
    ...
    def save(self):
        ...
        # 如果开启了 EMA，将优化器维护的各率 EMA 状态参数还原转换为对应权重并物理保存
        for i, ema_rate in enumerate(self.ema_rate):
        ...
        # 整合基础训练控制状态（优化器状态、当前训练步数、数据采样器等），以便后续恢复训练
        misc_ckpt = {
    ...
    def finetune_from(self, finetune_ckpt):
        ...
        for name, model in self.models.items():
            ...
            if name in finetune_ckpt:
                ...
                # 比较并校验加载的微调权重与当前模型的张量 Shape 是否匹配，如不匹配则保留当前模型权重以防报错
                ...
                model_ckpts[name] = model_ckpt
                model.load_state_dict(model_ckpt)
                # 若使用 inflat_all FP16 模式，将加载并覆盖后的子模型参数转换为半精度（FP16）
                if self.fp16_mode == 'inflat_all':
                    model.convert_to_fp16()
            else:
                if self.is_master:
        ...
        # 将各子模型最终加载的微调参数同步映射到优化器对应的主参数（Master Params）中
        self._state_dicts_to_master_params(self.master_params, model_ckpts)
        del model_ckpts

        # 多 GPU 分布式环境下各进程通过 barrier 进行同步
        if self.world_size > 1:
            dist.barrier()
        if self.is_master:
            print('Done.')

        # 启动 DDP 校验以检查各卡微调载入后参数是否完全一致
        if self.world_size > 1:
            self.check_ddp()
    ...
    def run_step(self, data_list):
        ...
        # 训练过程
        losses = []
        # BEGIN detailed loss export: collect per-microbatch loss snapshots for loss.txt.
        loss_exports = []
        # END detailed loss export.
        statuses = []
        ...
        for i, mb_data in enumerate(data_list):
            ...
            # BEGIN detailed loss export: copy loss details without mutating training loss.
            loss_exports.append(self.export_loss_for_file(loss))
            # END detailed loss export.
        ...
        # BEGIN detailed loss export: expose the averaged step loss snapshot to BaseTrainer.
        self._last_loss_export = dict_reduce(loss_exports, lambda x: np.mean(x))
        # END detailed loss export.
