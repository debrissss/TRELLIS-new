"""Prepare tiny FaceScape SS overfit datasets and training configs."""
...
BASE_CONFIG = PROJECT_ROOT / "configs" / "generation" / "ss_flow_img_dit_L_16l8_fp16_finetune_facescape.json"
...
def has_required_assets(row: dict[str, str], source_root: Path) -> bool:
    ...
    return (
        ...
        and (source_root / "ss_latents" / LATENT_MODEL / f"{sha}.npz").is_file()
    )
...
def prepare_subset(source_root: Path, subset_root: Path, rows: list[dict[str, str]]) -> None:
    ...
    for row in rows:
        ...
        replace_path_with_symlink(
            source_root / "ss_latents" / LATENT_MODEL / f"{sha}.npz",
            subset_root / "ss_latents" / LATENT_MODEL / f"{sha}.npz",
        )


def build_overfit_config(base_config: dict, max_steps: int, sample_count: int = 4) -> dict:
...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Prepare SS overfit datasets and configs.")
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    for count in args.counts:
        selected = select_neutral_rows(rows, args.source_train_root, count)
        name = f"overfit_{count}"
        subset_root = args.dataset_root / name
        config_path = args.config_root / f"ss_flow_facescape_{name}.json"
        output_dir = args.output_root / f"ss_flow_facescape_{name}"
        script_path = PROJECT_ROOT / "fine_tuning" / f"train_ss_flow_facescape_{name}.sh"
        ...
        write_json(config_path, build_overfit_config(base_config, args.max_steps, sample_count=count))
