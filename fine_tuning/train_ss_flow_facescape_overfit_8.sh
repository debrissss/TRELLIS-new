...
"${PYTHON}" train.py \
  --config /root/autodl-tmp/TRELLIS-new/configs/generation/overfit/ss_flow_facescape_overfit_8.json \
  --data_dir /root/autodl-tmp/TRELLIS-new/datasets/Facescape/overfit_8 \
  --output_dir /root/autodl-tmp/TRELLIS-new/outputs/ss_flow_facescape_overfit_8 \
  --num_gpus "${NUM_GPUS:-1}" \
  --ckpt none \
  "$@"
