...
"${PYTHON}" train.py \
  --config /root/autodl-tmp/TRELLIS-new/configs/generation/overfit/slat_flow_facescape_overfit_1.json \
  --data_dir /root/autodl-tmp/TRELLIS-new/datasets/Facescape/overfit_1 \
  --output_dir /root/autodl-tmp/TRELLIS-new/outputs/slat_flow_facescape_overfit_1 \
  --num_gpus "${NUM_GPUS:-1}" \
  --ckpt none \
  "$@"
